...
- [ ] Sinu kast jääb ise seisma imemisel ja puhumisel. USB välja, pump välja.
  ...
  - Puudu: Atomi enda prošivi (mitte käsitsi CLI/veebilehe) iseseisev peatamine — `logger.py` tuleb käivitada nende numbritega ilma `--dry-run`-ta; ja kümme võtmistsüklit robotiga (pumba töötsükkel) veel mõõtmata.
